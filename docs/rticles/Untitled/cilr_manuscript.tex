...
\tightlist
\item
  \(\mathbf{X}\): \(n\) by \(p\) matrix of positive counts for \(p\) taa
  and \(n\) samples measured through either targeted sequencing \(p\)
  matrix of positive counts for \(p\) taxa and \(n\) samples measured
  through either targeted sequencing (such as 16S rRNA) or whole genome
  shotgun sequencing. Usually \(\mathbf{X}\) is generated from standard
  sequence processing pipelines such as DADA2 \citep{callahan2016} and
  MetaPhlAn2 \citep{truong2015}
\end{itemize}
