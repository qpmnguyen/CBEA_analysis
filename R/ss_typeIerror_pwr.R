...
sim <- qread(file = glue("{dir}/fdr_sim/parameters.qs", dir = dir))
...
sim$eval <- future_map(1:nrow(sim), .f = ~{
  ...
  return(calculate_statistic(eval = opt$eval, pred = score))
...
with_progress({
  ...
  wc_eval <- future_map(1:nrow(sim), .f = ~{
    ...
    return(calculate_statistic(eval = "fdr", pred = label))
  })
})
plan(sequential)
