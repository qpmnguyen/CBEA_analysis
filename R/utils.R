...
#' This function is the same as generate_alt_scores but using physeq and BiocSets
#' instead
...
alt_scores_physeq <- function(physeq, set, method, preprocess, ...) {
    ...
    if (method %in% c("gsva", "ssgsea", "zscore", "plage")) {
        ...
        scores <- t(gsva(
            expr = X, gset.idx.list = set_list,
            method = method, kcdf = kernel
        ))
...
wc_test_physeq <- function(physeq, set, thresh = 0.05, alt = "two.sided", 
    ...
    scores <- map_dfr(set_list, ~{
        ...
        apply(X, 2, function(x){
            test_obj <- wilcox.test(x = x[idx], y = x[-idx], alternative = alt)
            if (output == "sig"){
                return(ifelse(test_obj$p.value <= thresh, 1, 0))
            } else if (output == "scores"){
                return(test_obj$statistic)
            } else if (output == "pval"){
                return(test_obj$p.value)
            }
        })
    }) 
    #scores <- tibble(
...
wc_test <- function(X, A, thresh, alt = "two.sided", preprocess = F,
    ...
    for (i in seq(ncol(A))) {
        R[, i] <- apply(X, 1, function(x) {
            test <- wilcox.test(
                x = x[which(A[, i] == 1)], y = x[which(A[, i] != 1)],
                alternative = alt, ...
            )
            if (output == "sig") {
                return(test$p.value)
...
get_fit <- function(data, adj, distr = c("norm", "mnorm"), init = NULL, ...) {
    ...
    output <- tibble(
        ks = do.call(ks.test, ks_param)$statistic,
