...
plot_df <- parameters %>% unnest(starts_with("fdr")) %>% 
  ...
  pivot_longer(starts_with("fdr")) %>% group_by(spar, b_rho, n_inflate, name)
...
plt <- ggplot(plot_df, aes(y = mean, x = as.factor(spar), col = name, group = name)) + geom_point() +
  scale_color_npg(labels = c("cILR Mixture Normal", "cILR normal", "cILR raw", "cILR skewed t", "Wilcoxon Rank Sum")) + 
...
plot_df <- parameters %>% unnest(starts_with("fdr")) %>% 
  ...
  pivot_longer(starts_with("pwr")) %>% group_by(spar, b_rho, eff_size, name) %>% unnest(value)
