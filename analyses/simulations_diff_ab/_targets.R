...
analysis <- tar_map(values = sim_grid, unlist = TRUE, names = c("id"), 
        ...
        tar_target(evaluation,{
            res <- eval_function(analysis)
            dplyr::bind_cols(id = id, sim_eval_grid, est = res)
        })
)

combined <- tar_combine(combined_results, analysis[[4]])
save_file <- tarchetypes::tar_rds(save_file, saveRDS(combined_results, file = "output/sim_diff_ab.rds"))

list(sim_eval_grid, analysis, combined, save_file)
