...
pwr_cilr <- tar_map(unlist = FALSE, values = cilr_settings,{
    tar_rep(pwr_cilr, {
        ...
        data.frame(pwr = pwr, models = models, distr = distr, adj = adj, output = "sig")
    }, batches = 10, reps = 10)
})
...
pwr_other <- tar_map(unlist = FALSE, values = auc_models, {
    tar_rep(pwr_models, {
        X <- data_enrich$X
        idx <- sample(1:nrow(X), size = nrow(X), replace = F)
        X_boot <- X[idx, ]
        label_boot <- data_enrich$label[idx]
        pwr <- enrichment_analysis(X = X_boot, A = data_enrich$A, method = "wilcox", label = label_boot)
        data.frame(pwr = pwr, models = "wilcox")
    }, batches = 10, reps = 10)
})

auc_combined <- tar_combine(auc, auc_cilr[[2]], auc_other[[2]], command = dplyr::bind_rows(!!!.x))
pwr_combined <- tar_combine(pwr, pwr_cilr[[2]], pwr_other[[2]], command = dplyr::bind_rows(!!!.x))
