...
generate_aggregation <- function(proc_object, level, method = c("cilr", "gsva", "clr", "ssgsea"), ...){
    proper_names <- c("data", "label", "table")
    ...
    A <- taxtab2A(proc_object$table, agg_level = level, full = FALSE)
    
    # Get matrix from data 
    data <- proc_object$data
    ...
    new_dat <- new_dat %>% mutate(label = proc_object$label) %>% 
